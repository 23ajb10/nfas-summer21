...
void Automata::automataToNFAFile(string out_fn) {
    ...
    queue<pair<string, int>> to_process;
    list<pair<string, int>> to_process_list;
    ...
    // output nfa is incorrect, not sure why
    // This numbering scheme is the same as #1, but instead of numbering symbol 0 through 255,
    // it starts with symbol 0 and then chooses each subsequent symbol by picking the first symbol
    // that has a transition from the same state that the previous symbol has a transition from.
    int states_remaining = starts.size();
    uint32_t curr_symbol = 0;
    int symbols_remaining = NUM_SYMBOLS;
    char start_bitmap[NUM_SYMBOLS/CHAR_BITS] = {0};
    

#define sym_bit  (1 << (curr_symbol % CHAR_BITS))
#define sym_chunk (curr_symbol / CHAR_BITS)


    // iterate through all symbols
    while (symbols_remaining > 0) {
        // set current symbol to used
        start_bitmap[sym_chunk] |= sym_bit;
        bool next_symbol_set = false;
        uint32_t next_symbol;
        // for each start state
        for (STE *start : starts) {
            if (marked[start->getId()]) {
                continue;
            }

            for (uint32_t j : start->getIntegerSymbolSet()) {
                // if current symbol is in set of state's symbols
                if (curr_symbol == j) {
                    ...
                    pair<string, int> element = make_pair(start->getId(), 0);
                    to_process_list.push_back(element);

                    if ((!next_symbol_set) && start->getIntegerSymbolSet().size() > 1) {
                        for (uint32_t ii : start->getIntegerSymbolSet()) {
                            char is_used = start_bitmap[ii/CHAR_BITS] & (1 << (ii % CHAR_BITS));
                            if ((!is_used) && ii != curr_symbol ) {
                                next_symbol = ii;
                                next_symbol_set = true;
                                break;
                            }
                        }
                    }
        ...
        if (!next_symbol_set) {
            // find next unused symbol if next symbol not set
            for (int j = 0; j < NUM_SYMBOLS/CHAR_BITS; j++ ) {
                for (int i = 0; i < CHAR_BITS; i++) {
                    char mask = 1 << i;
                    if ((start_bitmap[j] & mask) == 0) {
                        next_symbol = j*CHAR_BITS + i;
                        next_symbol_set = true;
                        break;
                    }
                }
    ...
    for(STE *start : starts) {
        ...
        if(id_map.find(start->getId()) == id_map.end()) {
            //str += "not all start states reached\n";
            id_map[start->getId()] = state_counter++;
        }
    ...
    while(!to_process.empty()) {
        ...
        // get an element
        pair<string, int> element = to_process.front();
        ...
        for(string s : ste->getOutputs()) {
            //push to todo list
            if(marked[s] != true)
                to_process.push(make_pair(s, level + 1));
        }
    }
    
    list<pair<string, int>>::iterator it;
    vector<string> level_vec;
    int current_level = 0;
    // iterate through states level by level
    for ( it = to_process_list.begin(); it != to_process_list.end(); it++ ) {
        pair<string, int> element = *it;
        
        // once we reach element at next level, process previous level vector       
        if (current_level != element.second)  {
            states_remaining = level_vec.size();
            curr_symbol = 0;
            symbols_remaining = NUM_SYMBOLS;
            char symbol_bitmap[NUM_SYMBOLS/CHAR_BITS] = {0};

            // process completed level
            while (symbols_remaining > 0) {
                symbol_bitmap[sym_chunk] |= sym_bit;
                uint32_t next_symbol;
                bool next_symbol_set = false;

                for ( string current : level_vec ) {
                    // go through each element in level
                    STE * ste = dynamic_cast<STE *>(getElement(current));
                    for (uint32_t j : ste->getIntegerSymbolSet()) {
                        if (curr_symbol == j) {
                            // if current symbol is in element's symbol set
                            
                            if (id_map.find(ste->getId()) == id_map.end()) {
                                // add to id_map and assign number if not already in it
                                id_map[ste->getId()] = state_counter++;
                                states_remaining--;
                            }

                            // if we have not assigned next symbol and there is more than one symbol in set
                            if ((!next_symbol_set) && ste->getIntegerSymbolSet().size() > 1) {
                                for (uint32_t ii : ste->getIntegerSymbolSet()) {
                                    char is_used = symbol_bitmap[ii/CHAR_BITS] & (1 << (ii % CHAR_BITS));
                                    if ((!is_used) && ii != curr_symbol ) {
                                        next_symbol = ii;
                                        next_symbol_set = true;
                                    break;
        ...
        level_vec.push_back(element.first);
    }
    //cout << "here" << endl;
    for (auto e : elements) {
        if (id_map.find(e.second->getId()) == id_map.end()) {
            id_map[e.second->getId()] = state_counter++;

        }
    }
    
    // now we have all states numbered correctly, must build string w/ transitions
    queue<string> to_process_string;
    ...
    for(STE *start : starts) {
        ...
        for(string s : start->getOutputs()) {
            ...
            //push to todo list if we haven't already been here
            if(visited[s] != true) {
                to_process_string.push(s);

            }
        }
    }
    cout << "here2" << endl;
    while(!to_process_string.empty()) {

        // get an element
        string id = to_process_string.front();
        //cout << elements[id]->toString() << endl;
        STE * ste = dynamic_cast<STE *>(getElement(id));
